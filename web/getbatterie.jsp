...
<html>
    <head>
        ...
        <title>Ajout/Modification d'une batterie</title>
    ...
    if(!act.isAdmin(session.getAttribute("user").toString())){
        ...
        rs = db.S.executeQuery("Select * from Batterie where idBatterie='"+ids+"'");
        if(rs.next()){
    ...
    <center>
        ...
        <div  style="padding-left: 20%;position: absolute;z-index: 3">
            <form method="post" action="actions?action=editbatterie&ids=<%=ids%>" >
        <table border="0" cellpadding="0" cellspacing="0"  id="id-form">
		<tr>
			<th valign="top">ID Batterie:</th>
			...
			<td><input type="text" name="txty" value="<%=vl.get(1)%>" id="txty" <%=dis%> <%=noedit%> class="inp-form"/></td>
			<td></td>
		</tr>
                <tr>
			<th valign="top">Marque:</th>
                        <td><input type="text" name="typ" value="<%=vl.get(2)%>" <%=dis%> <%=noedit%> class="inp-form" /></td>
			<td></td>
		</tr>
                <tr>
			<th valign="top">Capacité:</th>
                        <td><input type="text" name="cap" value="<%=vl.get(5)%>" <%=dis%> <%=noedit%> class="inp-form" /></td>
			<td></td>
		</tr>
                <tr>
			<th valign="top">Date mise en service:</th>
                        <td><input type="text" name="dm" value="<%=vl.get(6)%>" <%=dis%> <%=noedit%> class="inp-form" /></td>
			<td></td>
		</tr>
                <tr>
