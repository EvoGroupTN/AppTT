...
<html>
    ...
    <body onload="$('#step-holder').hide('slow')">
        ...
        if(request.getParameter("q")!=null){
            ...
            rs = db.S.executeQuery("Select * from users where login like '%"+q+"%' or login in (select login from UserSite where idsite like '%"+q+"%')");
            
                               }else{
            rs = db.S.executeQuery("select * from users");
                               }
        int i=0;
        ...
        while(rs.next()){
            ...
            <div id="<%=ids%>" <%=info3 %> class="contentrow">
                <h1><%=ids%></h1>
                <p class="txtinfo"><b>Type:</b> <%=info1%></p>
                <p class="txtinfo"><b>Description ou remarques:</b> <%=info%></p>
                
                    <div style="float: left;"><a  href='#' onclick="showsiteinfo('getuser.jsp?idb=<%=ids%>');">Afficher</a></div>&nbsp;<div style="float: left;<%=vis%>">&nbsp;| <a href='getuser.jsp?edit&idb=<%=ids%>' target='frminfo' onclick="showinfo()">Modifier</a> | <a href="actions?action=delete&what=users&ids=<%=ids%>" target='frmsub' onclick="loadtodiv('#<%=ids%>','done.html')">Supprimer</a></div><br/>
