...
<html>
    <head>
        ...
        <title>Ajout/Modification d'un BSC</title>
    ...
    if(!act.isAdmin(session.getAttribute("user").toString())){
        ...
        rs = db.S.executeQuery("Select * from BSC where idBSC='"+ids+"'");
        if(rs.next()){
    ...
    <center>
        ...
        <div  style="float: left;">
            <form method="post" action="actions?action=editbsc&ids=<%=ids%>" >
		...
		<tr>
			...
			<th valign="top">ID MSC:</th>
                        <td>
                            <select name="idmsc" style="width:100%" <%=dis%> <%=noedit%>> 
                                <%
                                    rs = db.S.executeQuery("Select idMSC from MSC");
                                    while(rs.next()){
                                %>
                                <option value="<%=rs.getString(1)%>" <% if(inf1.compareToIgnoreCase(rs.getString(1))==0) out.print("selected"); %>><%=rs.getString(1)%></option>
                                <%}%>
                            </select>
                        </td>
			<td></td>
			...
			<th valign="top">Description:</th>
                        <td>
                            <input type="text" name="desc" value="<%=inf2%>" id="txty" <%=dis%> class="inp-form"/>
                        </td>
                        <td ></td>
		</tr>
                <tr>
			<th valign="top">Date mise en service:</th>
			<td><input type="text" name="dmes" value="<%=inf3%>" id="txty" <%=dis%> class="inp-form"/></td>
			<td></td>
		</tr>
                <tr>
			<th valign="top">Configuration:</th>
                        <td><input type="text" name="conf" value="<%=inf5%>" <%=dis%> class="inp-form" /></td>
			<td></td>
		</tr>
